...
def get_monthly_results(goal_doctype, goal_field, date_col, filters, aggregation="sum"):
	...
	query = build_monthly_results_query(goal_doctype, table, goal_field, date_col, date_format, filters, aggregation)
	return dict(query.run())


def build_monthly_results_query(goal_doctype, table, goal_field, date_col, date_format, filters, aggregation):
	return (
		frappe.qb.get_query(
			table=goal_doctype,
			fields=[
...
def get_monthly_goal_graph_data(
	...
	return {
		"title": title,
		"data": {"datasets": [{"values": dataset_values, "formatted": values_formatted}], "labels": month_labels, **y_markers},
