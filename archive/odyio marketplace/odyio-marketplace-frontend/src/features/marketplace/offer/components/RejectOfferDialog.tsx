...
export default function RejectOfferDialog({
  ...
  const handleSubmit = () => {
    if (isTooLong || isPending) {
      return
    }

    onReject(trimmedReason || undefined)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 py-6">
      ...
      >
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600">
          Cette décision est définitive. Vous pouvez indiquer un motif au fournisseur.
        </p>

        {errorMessage ? (
          <div className="mt-4" aria-live="assertive">
            <ErrorState title="Refus impossible" message={errorMessage} />
          </div>
        ) : null}

        <label htmlFor={reasonId} className="mt-5 block">
          <span className="text-sm font-medium text-slate-700">Motif du refus — facultatif</span>
          <textarea
            id={reasonId}
            ref={textareaRef}
            value={reason}
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          >
            {isPending ? "Refus de l'offre..." : "Refuser l'offre"}
          </button>
        </div>
      </div>
    </div>
