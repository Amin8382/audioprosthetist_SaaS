...
public class FournisseurService {
    ...
    public List<BCLigne> getBcLignes(UUID bcId) {
        return bcLigneRepository.findByBcId(bcId);
    }

    public void verifyAccess(UUID bcId, UUID userId) {
        bcRepository.findById(bcId).orElseThrow(() -> new IllegalArgumentException("BC not found"));
    }

    public byte[] getBcPdf(UUID bcId) {
        BonCommande bc = bcRepository.findById(bcId).orElseThrow();
        List<BCLigne> lignes = getBcLignes(bcId);
        return pdfService.generateBcPdf(bc, lignes);
    }
