...
import com.odyio.document.DocumentType;
import com.odyio.document.OwnerType;
...
public class CatalogueController {
    ...
    @PostMapping(value = "/catalogue/{id}/image", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<Map<String, String>> uploadImage(
            @PathVariable UUID id, @RequestParam MultipartFile file, Authentication auth) {
        UUID userId = (UUID) auth.getPrincipal();
        Document doc = documentService.store(file, OwnerType.CATALOGUE, id, DocumentType.PRODUCT_IMAGE, userId);
        CatalogueProduit produit = catalogueService.findById(id);
        produit.setImagePath(doc.getFilePath());
        catalogueProduitRepository.save(produit);
        return ResponseEntity.ok(Map.of("documentId", doc.getId().toString()));
    }
}
