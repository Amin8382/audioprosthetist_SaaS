...
public class TresorerieController {
    ...
    @GetMapping("/bordereau/print")
    public ResponseEntity<byte[]> printBordereau(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate start,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate end) {
        byte[] pdf = tresorerieService.getBordereauPdf(start, end);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=\"BORDEREAU-" + start + "-" + end + ".pdf\"")
