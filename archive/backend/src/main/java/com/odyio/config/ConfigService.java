...
public class ConfigService {
    ...
    @Transactional
    public String uploadLogo(MultipartFile file, UUID userId) {
        ClinicConfig cfg = getConfig();

        if (cfg.getLogoPath() != null) {
            try {
                java.nio.file.Files.deleteIfExists(java.nio.file.Paths.get(cfg.getLogoPath()));
            } catch (java.io.IOException ignored) {}
        }

        Document doc = documentService.store(file, OwnerType.CONFIG,
            cfg.getId(), DocumentType.LOGO, userId);
        cfg.setLogoPath(doc.getFilePath());
    ...
    public String getLogoUrl() {
        ClinicConfig cfg = getConfig();
        if (cfg.getLogoPath() == null) return null;
        return cfg.getLogoPath();
    }
