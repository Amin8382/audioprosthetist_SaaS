...
export default function FournisseurForm() {
  ...
  useEffect(() => {
    if (id) {
      api.get(`/fournisseurs/${id}`).then(r => {
        const f = r.data;
        setForm({ name: f.name || '', contactName: f.contactName || '', phone: f.phone || '', email: f.email || '', address: f.address || '', notes: f.notes || '' });
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">{isEdit ? 'Modifier fournisseur' : 'Nouveau fournisseur'}</h1>
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border max-w-2xl space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" className="px-6 py-2 bg-primary-600 text-white rounded-lg">{isEdit ? 'Mettre à jour' : 'Enregistrer'}</button>
