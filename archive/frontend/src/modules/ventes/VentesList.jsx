            ...
            {bls.map(bl => (
              <tr key={bl.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right flex items-center gap-2 justify-end">
                  <PrintButton endpoint={`/bls/${bl.id}/print`} label={<Printer size={16} />} className="!p-1.5" />
                  <Link to={`/ventes/${bl.id}`} className="text-primary-600 hover:underline">Détail</Link>
                </td>
              </tr>
            ))}
            {bls.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">Aucun BL</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
