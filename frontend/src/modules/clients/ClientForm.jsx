...
export default function ClientForm() {
  ...
  const [form, setForm] = useState({ fullName: '', phone: '', email: '', address: '', nationalId: '', cnamNumber: '', cnamAffiliationType: 'NONE', cnamExpiry: '', dateOfBirth: '', notes: '' });
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm border max-w-2xl space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium mb-1">N° CNAM</label>
            <input value={form.cnamNumber} onChange={e => update('cnamNumber', e.target.value)} className="w-full px-3 py-2 border rounded-lg" /></div>
          <div><label className="block text-sm font-medium mb-1">Affiliation CNAM</label>
            <select value={form.cnamAffiliationType} onChange={e => update('cnamAffiliationType', e.target.value)} className="w-full px-3 py-2 border rounded-lg">
              <option value="NONE">Aucune</option><option value="CNAM_ACTIVE">CNAM Actif</option><option value="CNAM_RETIRED">CNAM Retraité</option><option value="PRIVATE">Privé</option>
            </select></div>
          <div><label className="block text-sm font-medium mb-1">Expiration CNAM</label>
            <input type="date" value={form.cnamExpiry} onChange={e => update('cnamExpiry', e.target.value)} className="w-full px-3 py-2 border rounded-lg" /></div>
        </div>
