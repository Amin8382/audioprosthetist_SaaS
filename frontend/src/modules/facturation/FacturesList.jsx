          ...
          <tbody>
            {factures.map(f => (
              <tr key={f.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right"><Link to={`/factures/${f.id}`} className="text-primary-600 hover:underline">Détail</Link></td>
              </tr>
            ))}
            {factures.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">Aucune facture</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
